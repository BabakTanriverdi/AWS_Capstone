...
aws ec2 run-instances --image-id ami-0aa210fd2121a98b7 --instance-type t3.micro --key-name mod_1_key --security-group-ids sg-04289b84caf5f4e25 --subnet-id subnet-09775c02103b80b1e --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=babak-aws-capstone-nat-instance}]' --disable-api-termination
```

- !!!!!! Warning!!!!!!select NAT instance and `enable stop` source/destination check
FOR THIS:

### Go to instance ---> Actions--> Networking --> Change source /Destination Check --> STOP --> SAVE
...
```bash
- eval $(ssh-agent)
- ssh-add <your pem file name with .pem>
- ssh -A ec2-user@<Public-IP of NAT Instance>

- When connected to the NAT instance:

- ssh ec2-user@<private IP of test instance>
...
- token: `TOKEN=$(aws --region=us-east-1 ssm get-parameter --name /<username>/capstone/token --with-decryption --query 'Parameter.Value' --output text)`
...
sudo ./aws/install
cd /home/ubuntu/
TOKEN=$(aws --region=us-east-1 ssm get-parameter --name /babak/capstone/token --with-decryption --query 'Parameter.Value' --output text)
...
1-Listener HTTP:80  ---> Default Action : choose Capstone Target grp(When you created Target grp before)
Add listener
2-Listener HTTPS:443 ---> Default Action : choose Capstone Target grp(When you created Target grp before)
```

- if you didn't created yet a Target Group
...
Brows load balancer and select your capstone ALB and choose
...
Next
TLS certificate Choose your certificate
Crate distribution
```
...
Now we'll create a lambda function with the python code given by developer team. Before we create our Lambda function, we should create IAM role that we'll use for Lambda function.
